$(document).ready(function(){
    $('#getGirlName').bind('click',getGirlName);
    $('#getBoyName').bind('click',getBoyName);
});
...
function getHightPoint(params){

    var reg = /"pingfen_right_ass"><span>(\d+)<\//g;
    console.log(params);
  //  $.post('http://www.sheup.com/xingming_dafen.php?xingx='+params.xingx+'&mingx='+params.mingx+'&Submit='+params.Submit,{},function(data){
    $.post('http://www.sheup.com/xingming_dafen.php',params,function(data){
        data = data.replace(/[\r\n\s+]/g,"")
        data = data.replace(/[ ]/g,"")
        var result = reg.exec(data)[1];
        if(parseInt(result)>94){
            var resultName = params.split('name')[1];
            var point = parseInt(result);
             alert(resultName+"分数："+point);
        }
    })
}
...
function getMing(sex){
    var nameAll
    if(sex =='girl'){
        girlData = girlData.replace(/[\s+]/g,"")
        nameAll = girlData
    }else{
        boyData = boyData.replace(/[\s+]/g,"")
        nameAll = boyData
    }
    var indexNum = Math.floor(Math.random()*nameAll.length);
    var indexNum2 =  Math.floor(Math.random()*nameAll.length);
    var mingzi = nameAll.substring(indexNum-1,indexNum)+nameAll.substring(indexNum2-1,indexNum2);
    return  mingzi;
